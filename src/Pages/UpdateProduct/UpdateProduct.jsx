...
const NewFeatureField = () => {
    const handleDeleteFeatureField = ( e ) => {
    ...
    return (
        <div className="form-control relative">
            ...
            />
            <img onClick={ handleDeleteFeatureField } src={ crossIcon } className="absolute -right-2 -top-2 h-5 w-5 rounded-full cursor-pointer" />
        </div>
    );
};
...
const UpdateProduct = () => {
    ...
    const addFeatureField = () => {
        setFeatureFields( ( previousFields ) => [ ...previousFields, <NewFeatureField /> ] );
    };
        ...
        Swal.fire( {
            ...
            if ( result.isConfirmed ) {
                (
                    async () => {
                        setLoading( true );
                        if ( formRef.current.picture.files[ 0 ] ) {
                            try {
                                ...
                                const serverResponse = await fetch( `${ import.meta.env.VITE_SERVERADDR }/product/${ product._id }`, {
                                    method: "PUT",
                                    headers: {
    ...
    return (
        <section>
            <h2 className="text-4xl font-bold py-6 text-center box-content">Update Product</h2>
            <form
                ref={ formRef }
            ...
            >
                ...
                <div class="form-control">
                    <label class="label">
                        <span class="label-text w-full">Update Thumbnail Image</span>
                    </label>
                    <input
                        name="picture"
                        type="file"
                ...
                <div className='lg:col-span-2 w-full flex flex-col gap-4 my-6'>
                    <div className="form-control">
                        <label className="input-group grid grid-cols-3">
                            ...
                            <input id='dimension' type="text" required defaultValue={ product?.attributes?.[ 0 ].value } placeholder="E.g. 3 x 4 x 5 cm" className="placeholder-gray-700 col-span-2 input input-bordered w-full !rounded-r" />
                        </label>
                    </div>
                    <div className="form-control">
                        <label className="input-group grid grid-cols-3">
                            ...
                            <input id='weight' type="text" required defaultValue={ product?.attributes?.[ 1 ].value } placeholder="E.g. 2 KG" className="placeholder-gray-700 col-span-2 input input-bordered w-full !rounded-r" />
                        </label>
                    </div>
                    <div className="form-control">
                        <label className="input-group grid grid-cols-3">
                            <span className='w-full bg-primary text-base-200 !rounded-l'>Country of Origin</span>
                            <input id='origin' type="text" defaultValue={ product?.attributes?.[ 2 ].value } placeholder="E.g. Made in Japan" className="placeholder-gray-700 col-span-2 input input-bordered w-full !rounded-r" />
                        </label>
                    </div>
                    <div className="form-control">
                        <label className="input-group grid grid-cols-3">
                            <span className='w-full bg-primary text-base-200 !rounded-l'>Model Number</span>
                            <input id='modelNumber' type="text" defaultValue={ product?.attributes?.[ 3 ].value } placeholder="E.g. JP-HCNXM2023" className="placeholder-gray-700 col-span-2 input input-bordered w-full !rounded-r" />
                        </label>
                    </div>
                </div>
                    ...
                    <div className='flex flex-col gap-4' ref={ featuresRef }>
                        {
                            ...
                            <div class="form-control relative">
                                <input
                                    ...
                                    class="input input-bordered w-full rounded placeholder-gray-700"
                                />
                                <img onClick={ handleDeleteFeatureField } src={ crossIcon } className="absolute -right-2 -top-2 h-5 w-5 rounded-full cursor-pointer" />
                            </div>
                        }
                        {
                            product?.features?.map( ( feature, idx ) => {
                                return (
                                    <div key={ idx } class="form-control relative">
                                        <input
                                            name="features"
                                            type="text"
                ...
                <button type='submit' className="btn btn-success rounded lg:col-span-2 mt-4 w-full">Update Details</button>
            </form>
        </section>
    );
};
