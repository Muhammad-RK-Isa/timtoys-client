    ...
    const handleSearch = ( e ) => {
        ...
        (
            async function () {
                try {
                    if ( string ) {
                        const response = await fetch( `${ import.meta.env.VITE_SERVERADDR }/products/search?page=${ page }&limit=20&string=${ encodeURIComponent( string ) }&sellerId=${ user.uid }` );
                        const data = await response.json();
                        setProducts( data );
                    } else {
                        const response = await fetch( `${ import.meta.env.VITE_SERVERADDR }/products/filter?page=${ page }&sortby=${ encodeURIComponent( selected ) }&limit=20&sellerId=${ user.uid }` );
                        const data = await response.json();
                        setProducts( data );
                    }
                }
    ...
    useEffect( () => {
        ...
        (
            async function () {
                setLoading( true );
                try {
                    const response = await fetch( `${ import.meta.env.VITE_SERVERADDR }/products/count?sellerId=${ user.uid }` );
                    const data = await response.json();
                    ;
                ...
                productsCount ?
                    ...
                    <div className='mb-20 w-full'>
                        <div className='grid grid-cols-5 lg:grid-cols-6 gap-4 items-center w-full py-4'>
                            ...
                            <h2 className='text-2xl col-span-2 lg:col-span-1'>My Toys</h2>
                        ...
                        {
                            cardView ?
                                <ProductsCards products={ products } entryAnimation="fade-up" response={ response } editable={ true } />
                                :
                                <ProductsList products={ products } entryAnimation="fade-up" response={ response } editable={ true } />
                        }
                    ...
                    :
                    <div className="h-[55vh] text-4xl font-semibold text-center grid place-content-center gap-8">
                        You haven't published any products yet.
                        <Link to='/user/products/add_product' className="underline text-primary text-2xl">Want to add a toy?</Link>
                    </div>
            }
        </>
    );
